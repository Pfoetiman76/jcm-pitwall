...
class SimInfo:
    ...
    def close(self):
        """Close memory map"""
        # JCM Pitwall: Aenderung gegenueber dem Original. Schlaegt __init__
        # fehl (Spiel laeuft nicht), existiert _lmu_data nicht und der
        # Destruktor warf einen zweiten, verwirrenden Fehler hinterher.
        if getattr(self, "_lmu_data", None) is None:
            self.LMUData = None
            return
        self.LMUData = None
