...
class PitwallClient:
    ...
    def send_lap(self, payload: dict, spooled: bool = False):
        ...
        if payload.get("is_valid") and lt:
            self._ref_lap = lt if self._ref_lap is None else min(self._ref_lap, lt)
        elif lt and self._ref_lap and (payload.get("is_inlap") or payload.get("is_outlap")):
            self.field.observe_pit_loss(lt, self._ref_lap,
                                        "in" if payload.get("is_inlap") else "out")
